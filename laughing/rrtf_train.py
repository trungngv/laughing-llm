...
# This code is modified to allow fine-tuning of Microsoft's phi-1.5 model.
# Source: https://github.com/GanjinZero/RRHF/blob/main/train.py
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # change the default to 1 because we're not using it
...
class RRHFTrainer(Trainer):
    ...
    def rrhf_loss(self, scores, idxs, rw_scores):
        # this only works with batchsize = 1
        ...
        # compute the loss weighted by the reward difference
        return -(diff[aval]*rw_diff[0][aval]).sum()
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # phi-1.5 does not accept attention mask during training
...
def train(model_args=None, data_args=None, training_args=None):
    """Modify some default parameters to make it work with phi-1.5
    """
    ...
    model = transformers.AutoModelForCausalLM.from_pretrained(
        ...
        torch_dtype="auto",
        trust_remote_code=True # for phi-1.5
    )

    # Use Lora so we can train on smaller GPU    
    if model_args.model_name_or_path == 'microsoft/phi-1_5':
        print('creating a peft model with lora (rank=1)...')
    ...
    trainer = RRHFTrainer(model=peft_model, tokenizer=tokenizer, args=training_args, **data_module)
